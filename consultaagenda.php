<!DOCTYPE html>
<html lang="pt-pt">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="icon" href="img/saúde.jpg">
   <link rel="stylesheet" href="consultaage.css">
    <title>Consultas marcadas</title>
</head>
<body>


    
<nav>

<div class="nav">

<img src="img/saúde.jpg">

<div class="texto">
<span> <strong> Consultas Reagendadas </strong> </span>
</div>

</div>

<div class="btn">
<a href="busca.php" class="bt">Voltar</a>
</div>

</nav>
    
<form>


<?php
echo "<table border=10>";
echo "<tr>";
echo "<th>Cod_reagendamento</th>";
echo "<th>Nome</th>";
echo "<th>Número</th>";
echo "<th>Hora</th>";
echo "<th>Data</th>";
echo "<th>Especialidade</th>";
echo "<th>Doctores</th>";
echo "<th>Bilhete</th>";
echo "<th>....................</th>";
echo "</tr>";


$Hostname="localhost";
$Usuario="root";
$Senha="";
$Nome_da_BD="centro";
$Link=new mysqli($Hostname,$Usuario,$Senha,$Nome_da_BD);
$sql = "SELECT * FROM reagendamento";
$resultado = mysqli_query($Link,$sql);

while ($registro = mysqli_fetch_array($resultado))
{
    $cod_reagendamento = $registro['cod_reagendamento'];
    $nome = $registro['nome'];
    $numero = $registro['numero'];
    $hora = $registro['hora'];
    $data = $registro['data'];
    $especialidade = $registro['especialidade'];
    $doctores = $registro['doctores'];
    $bilhete = $registro['bilhete'];
    echo "<tr>";
    echo "<td>".$cod_reagendamento."</td>";
    echo "<td>".$nome."</td>";
    echo "<td>".$numero."</td>";
    echo "<td>".$hora."</td>";
    echo "<td>".$data."</td>";
    echo "<td>".$especialidade."</td>";
    echo "<td>".$doctores."</td>";
    echo "<td>".$bilhete."</td>";
    echo "<th>
    <a href='deleteagenda.php?cod_reagendamento=$registro[cod_reagendamento]'>
    <svg xmlns='http://www.w3.org/2000/svg' width='16' height='16' fill='currentColor' class='apagar' viewBox='0 0 16 16'>
    <path d='M6.5 1h3a.5.5 0 0 1 .5.5v1H6v-1a.5.5 0 0 1 .5-.5ZM11 2.5v-1A1.5 1.5 0 0 0 9.5 0h-3A1.5 1.5 0 0 0 5 1.5v1H2.506a.58.58 0 0 0-.01 0H1.5a.5.5 0 0 0 0 1h.538l.853 10.66A2 2 0 0 0 4.885 16h6.23a2 2 0 0 0 1.994-1.84l.853-10.66h.538a.5.5 0 0 0 0-1h-.995a.59.59 0 0 0-.01 0H11Zm1.958 1-.846 10.58a1 1 0 0 1-.997.92h-6.23a1 1 0 0 1-.997-.92L3.042 3.5h9.916Zm-7.487 1a.5.5 0 0 1 .528.47l.5 8.5a.5.5 0 0 1-.998.06L5 5.03a.5.5 0 0 1 .47-.53Zm5.058 0a.5.5 0 0 1 .47.53l-.5 8.5a.5.5 0 1 1-.998-.06l.5-8.5a.5.5 0 0 1 .528-.47ZM8 4.5a.5.5 0 0 1 .5.5v8.5a.5.5 0 0 1-1 0V5a.5.5 0 0 1 .5-.5Z'/>
    </svg>
    </a>    
    </th>";

    echo "</tr>";
}

mysqli_close($Link);
echo "</table>";
?>

</form>


